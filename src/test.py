...
async def test_start():
    ...
    try:
        ...
        user=create_user(db=db,user=UserCreate(username="testuser_3",email="test__3@gmail.com",password="testpassword"))
        session_id=generate_session_id()
        update_current_active(db=db,user_id=user.user_id,session_id=session_id)
        session_data=get_session_data(db=db,current_user=user)
