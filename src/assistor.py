...
from src.crud import fetch_preferences, update_preferences, fetch_chat_content,fetch_chat_summary, update_chat_summary, get_convo_count, update_convo_count, increment_convo_count,store_chat
    ...
    def build_consolidation_message(self,state:AssistState):
        ...
        try:
            ...
            for m in console_window:
                #print(type(m))
                chat+=f"USER: {m.get('user')}\n"
                chat+=f"ASSISTANT: {m.get('assistant')}\n"
            #print(chat)
            prev_chat_summary=state.prev_chat_summary
            if prev_chat_summary is not None:
                chat+=f"Previous summary of the chat : {prev_chat_summary}\n\n Recent conversation : {chat}"
            msg.append({
                'role':'user',
...
class OutputProcessor():
    ...
    def check_if_ready_for_out():
        pass
    def parse_json(self,raw_content:str):
    ...
    def process(self,state:AssistState):
        ...
        state.intent = self.parse_json(raw_content)
        intent=state.intent
...
class StateRepo:
    ...
    def construct_console_window(self,db:Session,state:AssistState,user:User):
        return fetch_chat_content(db=db,user_id=user.user_id,chat_id=state.chat_id,limit=state.convo_count,offset=0)
    ...
    def update_user_preferences(self,db:Session,user:User,preferences:dict):
        update_preferences(db=db,current_user=user,preference_blog=preferences)
class MemoryConsolidator:
    def __init__(self,
                in_processor:InputProcessor,
    ...
    async def consolidate_memory(self,db:Session,user:User,state:AssistState):
        print("consolidation begin build")
        msg=self.in_processor.build_consolidation_message(state=state)
        #print(msg)
        print("build complete llm response sending")
        responce = await self.llm_interface.summarize(msg)
        print("llm responce recieved")
        if 'message' in responce and 'content' in responce['message']:
            print(responce['message']['content'])
            inference=self.out_processor.parse_json(responce['message']['content'])
            #print(inference)
            print("state repo update pref")
            self.state_repo.update_user_preferences(db=db,user=user,preferences=inference.get('user_preferences',{}))
            print("state repo update chat summaries")
            self.state_repo.update_chat_summary(db=db,user=user,state=state,summary=inference.get('chat_summary',""))
            print("state repo update chat summary")
...
class StateManager:
    def __init__(self,
                max_steps:int,
                consolidation_bound:int,
                state_repo:StateRepo,
                memory_consolidator:MemoryConsolidator,
                loop_manager:FlowManager
                ):
    ...
    def get_state(self,db:Session,user:User,chat_id: Text,session_data:UserActive)->AssistState:
        ...
        if chat_id is None:
            ...
            print(state.convo_count)
            state.consol_window=self.state_repo.construct_console_window(db=db,user=user,state=state)
